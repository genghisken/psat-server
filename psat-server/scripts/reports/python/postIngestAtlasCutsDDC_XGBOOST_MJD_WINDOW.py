...
Usage:
  %s <configfile> [<candidate>...] [--datethreshold=<datethreshold>] [--update] [--recent] [--useflagdate]
  %s (-h | --help)
  %s --version
...
Options:
  ...
  --useflagdate                     Apply the cuts with the MJD window set around the original flag date (for testing).
...
import logging
# Added the following function so we can get the followup_flag_date while testing an object.
sys.path.append('../../common/python')
from queries import getAtlasObjects
...
SKYCELL_REGEX = r'skycell\.([0-9]+(\.[0-9]+){0,1})(\.[S|W]S){0,1}\.dif\.'
...
# ---------------------------------------------------------------------------
# 2026-05-21 PRODUCTION XGBOOST GATING BLOCK
# Aggregate gating using XGBoost P(real). The official promotion path uses the
# MJD-window mean P(real) with the legacy DDC cuts retained below as legacy code.
# ---------------------------------------------------------------------------
XGBOOST_FEATURES = ('real', 'var', 'lin', 'flaw', 'cr')
XGBOOST_REAL_MIN_DET = 0.01  # per-detection floor; detection must have P(real) > this
XGBOOST_REAL_THRESH = 0.2     # MJD-window mean P(real) gate for promotion
XGBOOST_MODEL_FILENAME = 'xgboost_hist_model.json'
_XGBOOST_MODEL = None
...
def checkDeltas(lcData, minpoints, mjddiff, mjddiffmax = 1000, mjdWindow = 6, followupFlagDate = None):
    ...
    # 2026-05-01 KWS Fixed bug - can't remove list items while iterating over the list.
    mjds = [m for m in mjds if m >= maxMJD - mjdWindow]
   ...
   try:
      ...
      if mjdThreshold is not None:
          cursor.execute ("""
                ...
                LEFT JOIN atlas_detectionsnnc n
                  on n.detection_id = d.id
...
# ---------------------------------------------------------------------------
# 2026-05-21 PRODUCTION XGBOOST GATING HELPERS
# These helpers keep the original object-level cuts but replace the legacy DDC
# per-detection gate with an aggregate MJD-window XGBoost P(real) gate.
# ---------------------------------------------------------------------------
def loadXGBoostModel():
   ...
   if _XGBOOST_MODEL is None:
      try:
         ...
         raise RuntimeError("xgboost is required for the production XGBoost gate") from e

      modelPath = os.path.join(os.path.dirname(os.path.abspath(__file__)), XGBOOST_MODEL_FILENAME)
...
def getXGBoostFeaturesFromObjectRow(objectRow):
   features = []

   for feature in XGBOOST_FEATURES:
      value = objectRow[feature]
      if value is None:
         return None
      try:
         value = float(value)
   ...
   return n.array(features, dtype=n.float32).reshape(1, -1)


def computeXGBoostPrealFromObjectRow(objectRow):
   features = getXGBoostFeaturesFromObjectRow(objectRow)
   if features is None:
      return None

   model = loadXGBoostModel()
   return float(model.predict_proba(features)[0, 1])


def evaluateDetectionGates(objectRow, detInMaskArea, minSize):
   sharedFailReasons = []

   if 'MM' in objectRow['object']:
      sharedFailReasons.append('mount_model_object')
   if 'test' in objectRow['object']:
      sharedFailReasons.append('test_object')
   if objectRow['pkn'] >= PMV_THRESH:
      sharedFailReasons.append('known_or_suspected_mover')
   if objectRow['det'] == 5:
      sharedFailReasons.append('negative_flux_det5')
   if objectRow['dup'] < 0:
      sharedFailReasons.append('negative_dup')
   if objectRow['mag'] <= 0:
      sharedFailReasons.append('non_positive_mag')
   if objectRow['pbn'] >= PBN_THRESH:
      sharedFailReasons.append('pbn_too_high')
   if objectRow['Filter'] == 't':
      sharedFailReasons.append('t_filter')
   if objectRow['x'] <= minSize:
      sharedFailReasons.append('x_near_left_edge')
   if objectRow['x'] >= (objectRow['nx'] - minSize):
      sharedFailReasons.append('x_near_right_edge')
   if objectRow['y'] <= minSize:
      sharedFailReasons.append('y_near_bottom_edge')
   if objectRow['y'] >= (objectRow['ny'] - minSize):
      sharedFailReasons.append('y_near_top_edge')
   if detInMaskArea:
      sharedFailReasons.append('heatmap_mask')

   legacyDDCClausePass = (
      (objectRow['pvr'] >= PVR_THRESH or objectRow['ptr'] >= PTR_THRESH) or
      ((objectRow['pvr'] > 0 or objectRow['ptr'] > 0) and objectRow['psc'] > 0) or
      ((objectRow['pvr'] >= 0 or objectRow['ptr'] >= 0) and (objectRow['psc'] > PSC_THRESH and objectRow['psc'] < 900))
   )

   hasNNC = not (
      objectRow['real'] is None and
      objectRow['var'] is None and
      objectRow['lin'] is None and
      objectRow['flaw'] is None and
      objectRow['cr'] is None
   )

   legacyFailReasons = list(sharedFailReasons)
   if not legacyDDCClausePass:
      legacyFailReasons.append('legacy_pvr_ptr_psc_clause')

   # pkn/pbn are legacy DDC cuts only; XGBoost uses NNC scores + model P(real).
   xgboostFailReasons = []
   xgboostPreal = None
   if 'MM' in objectRow['object']:
      xgboostFailReasons.append('mount_model_object')
   if 'test' in objectRow['object']:
      xgboostFailReasons.append('test_object')
   if objectRow['det'] == 5:
      xgboostFailReasons.append('negative_flux_det5')
   if objectRow['dup'] < 0:
      xgboostFailReasons.append('negative_dup')
   if objectRow['mag'] <= 0:
      xgboostFailReasons.append('non_positive_mag')
   if objectRow['Filter'] == 't':
      xgboostFailReasons.append('t_filter')
   if objectRow['x'] <= minSize:
      xgboostFailReasons.append('x_near_left_edge')
   if objectRow['x'] >= (objectRow['nx'] - minSize):
      xgboostFailReasons.append('x_near_right_edge')
   if objectRow['y'] <= minSize:
      xgboostFailReasons.append('y_near_bottom_edge')
   if objectRow['y'] >= (objectRow['ny'] - minSize):
   ...
   if not xgboostFailReasons:
      if not hasNNC:
         ...
         xgboostPreal = computeXGBoostPrealFromObjectRow(objectRow)
         if xgboostPreal is None:
            xgboostFailReasons.append('invalid_xgboost_features')
         elif xgboostPreal <= XGBOOST_REAL_MIN_DET:
            xgboostFailReasons.append('xgboost_preal_below_min_det')

   return {
      'xgboostPreal': xgboostPreal,
      'legacyPass': len(legacyFailReasons) == 0,
      'xgboostPass': len(xgboostFailReasons) == 0,
      'legacyFailReasons': legacyFailReasons,
      'xgboostFailReasons': xgboostFailReasons,
   }


def evaluateXGBoostCandidate(objectId, objectClassification, detectionList, cleanedObjectInfo, mjds,
                             mjdWindow = MJD_WINDOW, debug = True, checkTriggerIsRecent = False,
                             followupFlagMJD = None, methodLabel = 'XGBOOST'):
   # Keep the XGBoost path isolated from any caller-side list mutation.
   detectionList = list(detectionList)
   cleanedObjectInfo = list(cleanedObjectInfo)
   mjds = [list(mjd) for mjd in mjds]

   result = {
      'method': methodLabel,
      'promote': False,
      'triggerMJD': None,
      'qtpCount': len(cleanedObjectInfo),
      'passesMJDCheck': False,
      'passesMJDCountCheck': False,
      'rms': None,
      'windowDetect': 0,
      'filterSwitch': 0,
      'postDetectionFailReasons': [],
   }

   if result['qtpCount'] < RECURRENCE_THRESH:
      result['postDetectionFailReasons'].append('insufficient_quality_detections')
      return result

   if debug:
      print("")
      print("======")
      print("%s Candidate ID = %s (%s). QTPCount = %d." % (methodLabel, objectId, objectClassification, result['qtpCount']))

   passesMJDCheck = checkDeltas(mjds, RECURRENCE_THRESH, MIN_MJD_DIFF,
                                mjddiffmax = INTRA_DAY_THRESH_DAYS/2.0,
                                mjdWindow = MJD_WINDOW)
   result['passesMJDCheck'] = passesMJDCheck
   if debug:
      if passesMJDCheck:
         print("\t%s Minimum MJD gap is acceptable (pass)" % methodLabel)
      else:
         print("\t%s Minimum MJD gap not large enough (fail)" % methodLabel)

   rms = calculateRMSScatter(cleanedObjectInfo)
   result['rms'] = rms
   if debug:
      if rms > RMS_SCATTER_THRESH:
         print("\t%s rms position scatter = %f (too high - fail)" % (methodLabel, rms))
      else:
         print("\t%s rms position scatter = %f (pass)" % (methodLabel, rms))
   ...
   if checkTriggerIsRecent:
      ...
      if triggerMJD < mjd - mjdWindow*2:
         if debug:
            print("\t%s Trigger MJD is not recent (fail)" % methodLabel)
         passesMJDCountCheck = False
         result['postDetectionFailReasons'].append('trigger_not_recent')

   result['passesMJDCountCheck'] = passesMJDCountCheck
   result['triggerMJD'] = triggerMJD
   if debug:
      if passesMJDCountCheck:
         print("\t%s MJD count is acceptable (pass)" % methodLabel)
         print("\t%s Trigger MJD = %d" % (methodLabel, triggerMJD))
      else:
         print("\t%s MJD count not acceptable (fail)" % methodLabel)
   ...
   result['windowDetect'] = windowDetect
   result['filterSwitch'] = filterSwitch

   if not passesMJDCheck:
      result['postDetectionFailReasons'].append('mjd_gap')
   if rms >= RMS_SCATTER_THRESH:
      result['postDetectionFailReasons'].append('rms_scatter')
   if windowDetect != 1:
      result['postDetectionFailReasons'].append('window_detections')
   if not filterSwitch:
      result['postDetectionFailReasons'].append('filters')
   if not passesMJDCountCheck and 'trigger_not_recent' not in result['postDetectionFailReasons']:
      result['postDetectionFailReasons'].append('mjd_count')

   if debug:
      print("\t%s detection array = " % methodLabel, end=' ')
      for detectionRow in detectionList:
         sys.stdout.write("%d" % detectionRow[1])
      print("")

      print("\t%s    filter array = " % methodLabel, end=' ')
      for detectionRow in detectionList:
         sys.stdout.write("%s" % detectionRow[0]["Filter"])
      print("")

      print("\t%s %s --> " % (methodLabel, objectId), end=' ')
      if rms < RMS_SCATTER_THRESH:
         print("RMS : 1,", end=' ')
      else:
         print("RMS : 0,", end=' ')
...
def getRecentXGBoostPredictionRows(predictionRows, mjdWindow = MJD_WINDOW):
   if not predictionRows:
      return []

   maxMJD = max([row['mjd'] for row in predictionRows])
   return [row for row in predictionRows if row['mjd'] >= maxMJD - mjdWindow]


def getXGBoostAverageStats(predictionRows):
   prealValues = [row['preal'] for row in predictionRows]
   meanPreal = None
   medianPreal = None
   if len(prealValues) > 0:
      meanPreal = float(n.mean(prealValues))
      medianPreal = float(n.median(prealValues))

   return {
      'count': len(predictionRows),
      'valuesUsed': len(prealValues),
      'mean': meanPreal,
      'median': medianPreal,
      'passesThreshold': meanPreal is not None and meanPreal >= XGBOOST_REAL_THRESH,
      'threshold': XGBOOST_REAL_THRESH,
      'predictions': list(predictionRows),
   }


def getMJDWindowXGBoostAverageStats(predictionRows, mjdWindow = MJD_WINDOW):
   return getXGBoostAverageStats(getRecentXGBoostPredictionRows(predictionRows, mjdWindow = mjdWindow))


def applyXGBoostAverageGate(candidateResult, methodLabel, averageStats):
   result = dict(candidateResult)
   result['method'] = methodLabel
   result['postDetectionFailReasons'] = list(candidateResult.get('postDetectionFailReasons', []))
   result['xgboostAverageStats'] = averageStats
   result['xgboostPrealThreshold'] = XGBOOST_REAL_THRESH

   if not averageStats['passesThreshold']:
      result['promote'] = False
      reason = 'xgboost_average_unavailable'
      if averageStats['mean'] is not None:
         reason = 'xgboost_average_below_threshold'
      if reason not in result['postDetectionFailReasons']:
         result['postDetectionFailReasons'].append(reason)
...
#                to MJD of course).
# 2026-06-11 KWS Get the followup_flag_date from the originally promoted object so we can
#                test whether objects previous promoted would still be promoted using the
#                same time window.
def testObject(conn, object, mjdWindow = MJD_WINDOW, debug = False, checkTriggerIsRecent = False, followupFlagDate = None, masks = None, useFlagDate = None):
   ...
   # Grab the followup_flag_date
   if followupFlagDate is None and useFlagDate:
       # Get the original flag date from the database.
       summaryInfo = getAtlasObjects(conn, objectId=object)
       followupFlagDate = summaryInfo['followup_flag_date'].strftime("%Y-%m-%d %H:%M:%S")
       print(followupFlagDate)

   if followupFlagDate is not None:
       #followupFlagMJD = getMJDFromSqlDate(followupFlagDate + ' 00:00:00') + 1
       followupFlagMJD = getMJDFromSqlDate(followupFlagDate) + 1
   ...
   cleanedObjectInfo = []
   mjds = []
   xgboostDetectionList = []
   xgboostCleanedObjectInfo = []
   xgboostMjds = []
   xgboostPassingPredictionRows = []
   passesMJDCheck = False
   passesMJDCountCheck = False
   triggerMJD = None
   rms = None
   ...
   for objectRow in objectInfo:
      ...
      gateEvaluation = evaluateDetectionGates(objectRow, detInMaskArea, MIN_SIZE)

      if gateEvaluation['xgboostPreal'] is not None:
         predictionRow = {
            'mjd': objectRow["MJD"],
            'preal': gateEvaluation['xgboostPreal'],
            'pass': gateEvaluation['xgboostPass'],
         }

         if gateEvaluation['xgboostPass']:
            xgboostPassingPredictionRows.append(predictionRow)

      if gateEvaluation['xgboostPass']:
          xgboostMjds.append([objectRow["MJD"]])
          xgboostDetectionList.append((objectRow, detectionFlag))
          xgboostCleanedObjectInfo.append(objectRow)
   ...
   if qtpCount >= RECURRENCE_THRESH:
      ...
      if rms < RMS_SCATTER_THRESH and windowDetect == 1 and filterSwitch and passesMJDCheck and passesMJDCountCheck:
         if debug:
            print(" ---> passed threshold cuts")
         # Legacy DDC result retained for reference only. The official
         # promotion decision is assigned by the MJD-window XGBoost gate below.
         #objectsPromoted += 1
      else:
         if debug:
            print(" ---> failed threshold cuts")
   ...
   # ------------------------------------------------------------------------
   # 2026-05-21 PRODUCTION XGBOOST GATING BLOCK
   # Evaluate the exact same downstream aggregation steps using the XGBoost
   # detections, then require the recent MJD-window mean P(real) to pass 0.2.
   # ------------------------------------------------------------------------
   xgboostBaseResult = evaluateXGBoostCandidate(
      object,
      ...
      mjdWindow = mjdWindow,
      debug = False,
      checkTriggerIsRecent = checkTriggerIsRecent,
      followupFlagMJD = followupFlagMJD,
      methodLabel = 'XGBOOST_BASE'
   )
   xgboostMJDWindowResult = applyXGBoostAverageGate(
      xgboostBaseResult,
      'XGBOOST_MJD_WINDOW_MEAN',
      getMJDWindowXGBoostAverageStats(xgboostPassingPredictionRows, mjdWindow = mjdWindow)
   )

   objectForUpdate["promote"] = xgboostMJDWindowResult["promote"]
   objectForUpdate["triggerMJD"] = xgboostMJDWindowResult["triggerMJD"]
...
def applyAtlasDiffCuts(conn, dateTime, objectList, recent = False, flagDate = None, masks = None, useFlagDate = False):
   """
   ...
   for object in objectList:
      objectForUpdate = testObject(conn, object, checkTriggerIsRecent = recent, followupFlagDate = flagDate, masks = masks, useFlagDate = useFlagDate)
...
def main(argv = None):
    ...
    objectListForUpdate = applyAtlasDiffCuts(conn, dateTime, objectList, recent = recent, masks = siteMasks, useFlagDate = options.useflagdate)
