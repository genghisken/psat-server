...
Usage:
  %s <configfile> [<candidate>...] [--datethreshold=<datethreshold>] [--update] [--recent] [--loglocation=<loglocation>] [--logprefix=<logprefix>] [--logprefixlargedataset=<logprefixlargedataset>] [--logprefixeliminate=<logprefixeliminate>] [--useflagdate]
  %s (-h | --help)
  %s --version
...
from postIngestAtlasCutsDDC_XGBOOST_MJD_WINDOW import getAtlasObjectsToCheck, getMaxFollowupId, promoteObjectToEyeballList, applyAtlasDiffCuts, getMostRecentProcessedDate, checkIsObjectAlreadyPromoted, getSiteMasks
...
def worker(num, db, objectListFragment, dateAndTime, firstPass, miscParameters, q):
    ...
    objectsForUpdate = applyAtlasDiffCuts(conn, dateAndTime, objectListFragment, recent = recent, masks = siteMasks, useFlagDate = options.useflagdate)
