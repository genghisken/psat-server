...
Options:
  ...
  --shiftcut=<d>          Pixel shift threshold [default: 1.0].
  --sigma=<g>             Gaussian filter sigma [default: 1.5].
...
def assess_shift(img_ref, img_new, *,
    ...
    half   = fit_window // 2
    y0, y1, x0, x1 = py-half, py+half+1, px-half, px+half+1
    ...
    if make_plots:
        ...
        # Row 0: reference, target, corr surface
        ...
        # Row 1: fit window, residuals, 1D slice
        ...
        if residual_mode.lower() == "hist":
            ax11.hist(res_img.ravel(), bins=30, color='slategray')
        ...
        ax12.axhline(off + noise_sig * prominence_cut,
                     ls='--', lw=1, c='purple',
                     label=f'{prominence_cut} σ cut')
        ax12.set_title('Central row through peak')
        ax12.legend(fontsize=8)

        # Row 2: decision dashboard
        ...
        dash = (f"SNR               : {snr:.2f}\n"
                ...
                f"Decision          : {verdict}")
        ax20.text(0.01, 0.98, dash, va='top', ha='left',
...
def main():
    args = docopt(__doc__)
    ref_path   = args['<ref_fits>']
    tgt_path   = args['<target_fits>']
    profile    = args['--profile']
    prominence = float(args['--prominence'])
    shiftcut   = float(args['--shiftcut'])
    sigma      = float(args['--sigma'])
    fitwin     = int(args['--fitwin'])
    nan_policy = args['--nan']
    resid_mode = args['--resid']
    plot       = args['--plot']
