...
/* 2024-02-28 KWS Because vmax, vmin and outfile are also used in FITS2jpeg.c,
                  but are NOT the same variables, I had to rename them here
                  with an underscore. I suspect previous C compilers didn't
                  care but the latest Ubuntu 22.04 one does. */
...
/*--------------------------------------------------------------------*/
{
  ...
  /* open output file */
  if ((outfile_ = fopen(name, "wb")) == NULL) {
  ...
  jpeg_stdio_dest(&cinfo, outfile_);
...
/*--------------------------------------------------------------------*/
{
  ...
  for (i=0;i<nx;i++) {
    ...
    if (val==blank) { /* blanked */
      ...
      { /* non blanked */
	if (val<vmin_) val=vmin_;
	if (val>vmax_) val=vmax_;
	if (nonlinear)  {/* nonlinear */
	  arg = ((val-vmin_) * irange);
